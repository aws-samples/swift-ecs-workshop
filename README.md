...
1. Creating an app using Mobile Hub with AWS Cognito for User-sign in
...
3. The welcome page for Mobile hub will show you an option to create a new project. Alternatively, if you have already use Mobile Hub it will
show the list of projects that you had created.
4. Let us name the project “Intent 2016”.
5. Once you create the project the next page show a cards layout wherein
you will see AWS services that can be configured and added to your app.
6. We want to add a simple login using Facebook to our sample application.
We will be using AWS Cognito for this.
7. Click on User-sign in card.
8. You should see 3 options with Facebook, Google and Custom offered as
choices. Select Facebook.
9. If you click on the documentation column on the right side it will show
you the instructions to enable Facebook developer account.
10. Follow the steps for enabling Facebook developer account.
11. Once Facebook developer account is activated we want to copy the App id
of our sample application so that we can use Facebook login in our
sample app.
...
15. Download the app.
2. Adding more UI and code to the downloaded app to enable listing a product listing.
1. Open the downloaded app in Xcode.
2. The project view should look something like this:
3. Run the app using iPhone 6s emulator.
4. Once the app is up and running you will find that it has one existing
option, which indicates the user sign in process through Cognito.
5. We want to add another option to the app main page. This page when clicked will show up the product listing. We will add the corresponding code to the web service, which will return a JSON data getting serialized
to an object and display in this new page.
6. Download the code of the UI from:
https://code.amazon.com/packages/Amazon-ecs-swift-
workshop/trees/mainline/--/lab3
7. Copy the folder ProductListing from the downloaded app and paste in
your Xcode under the folder “Demo”.
  
8. Replace the code for Demo/MySampleApp/App/MainViewController.swift with the one we downloaded in step 6. Here we have only added the code for the creating the UI for adding a new option “Access Swift DB”
9. Build the project.
10. Run it for iPhone 6s .
11. First sign in using Facebook.
12. Go back to main page and click on “Access Swift DB”.

##Lab 3: Build and test mobile app using Amazon Device Farm

1. First we want to retrieve the .ipa file of the app that we just built.
2. In Xcode, first build the app for a generic device type by selecting
Produtct Build
3. This should generate a MySample.app file under Products folder.
4. Right click on the MySample.app file and select “Show in Finder”.
5. Create an empty folder and call it “Payload” (case-sensitive).
6. Copy the MySample.app file in the “Payload” folder.
7. Archive “Payload” folder and rename the zip file to Sample.ipa
8. We now have the ipa file which we will upload to AWS Device Farm and
run tests against.
9. Open aws.amazon.com console.
10. On the console home page under Mobile Services select Device Farm.
11. Create a new Project. Give it a name “Intent 2016”.
12. Under project “Intent 2016” click on “Create a new Run”.
13. First we want to upload our iOS app. Click on the Android/Apple logo
button.
14. Next click on the “Upload” button. This will open a Finder window, which
will ask for the ipa file. Select our Sample.ipa file.
15. Next click on “Next step”.
16. By default the test type selected will be “Built-in: Fuzz”. Click on “Next
step”
17. On the Select devices page it will show the top pool of devices. We will
work with this pool. Click on “Next Step”.
18. On the “Specify device state” page we do not want to change anything.
Click on “Review and start run”.
19. The tests will run for around 15 mins.
20. Once the tests are finished you can view the detailed report of the test
run.
